...
export default function SetUserRole({user, isAdmin, fetchData}) {

    const setAsAdminToggle = (userId) => {
        fetch(`http://localhost:4003/b3/users/${userId}/set-as-admin`, {
            method: 'PUT',
            headers: {
    ...
    const setAsCustomerToggle = (userId) => {
    fetch(`http://localhost:4003/b3/users/:userId/set-as-customer`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${localStorage.getItem('token')}`
        ...
        <>
            { isAdmin ?
                <button className="btn btn-error text-white" onClick={() => setAsCustomerToggle(user)}>
                    ...
                    <span className="hidden md:block text-white">Set as Customer</span>
                </button>
                :
                <button className="btn btn-success text-white" onClick={() => setAsAdminToggle(user)}>
                    ...
                    <span className="hidden md:block text-white">Set as Admin</span>
                </button>
            }
        </>
    );
